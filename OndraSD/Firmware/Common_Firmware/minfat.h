...
typedef struct
{
    __uint32_t sector;          /* sector index in file */
    __uint32_t cluster;         /* current cluster */
    __uint32_t size;            /* file size */
    __uint32_t firstcluster;
	__uint32_t cursor;	/* Offset within the current sector */
...
//	struct PartitionEntry Partition[4];	// We copy these (and byteswap if need be)
	__uint16_t Signature;		// This lets us detect an MBR (and the need for byteswapping).
} __attribute__ ((packed));
...
#define LCASE_EXT       0x10                /* filename extension in lower case */
    ...
    __uint16_t      CreateTime;         /* create time */
    __uint16_t      CreateDate;         /* create date */
    __uint16_t      AccessDate;         /* access date */
    __uint16_t      HighCluster;        /* high bytes of cluster number */
    __uint16_t      ModifyTime;         /* last update time */
    __uint16_t      ModifyDate;         /* last update date */
    __uint16_t      StartCluster;       /* starting cluster of file */
    __uint32_t       FileSize;           /* size of file in bytes */
} DIRENTRY;

typedef union {
    __uint16_t fat16[256];
    __uint32_t  fat32[128];
} FATBUFFER;
...
extern __uint32_t cluster_size;
extern __uint32_t cluster_mask;
extern __uint32_t fat32;
...
//unsigned char FileReadEx(fileTYPE *file, unsigned char *pBuffer, __uint32_t nSize);
