...
__uint32_t fat32;                // volume format is FAT32
__uint32_t fat_start;                // start LBA of first FAT table
__uint32_t data_start;               // start LBA of data field
__uint32_t root_directory_cluster;   // root directory cluster (used in FAT32)
__uint32_t root_directory_start;     // start LBA of directory table
__uint32_t root_directory_size;      // size of directory region in sectors
__uint32_t cluster_size;             // size of a cluster in sectors
__uint32_t cluster_mask;             // binary mask of cluster number
__uint32_t dir_entries;             // number of entry's in directory table

__uint32_t cachedsector=-1;

__uint32_t current_directory_cluster;
__uint32_t current_directory_start;
...
// __uint32_t buffered_fat_index;       // index of buffered FAT sector
...
int fileSearchMatchX(const char *fn)
{
	return 1;
}
...
{
	__uint32_t fat_number;
	__uint32_t fat_size;
	__uint32_t boot_sector;
	int partitioncount;
...
__uint32_t GetCluster(__uint32_t cluster)
{
	__uint32_t i;
	__uint32_t sb;
    if (fat32)
...
unsigned int FileOpen(fileTYPE *file, const char *name)
{
	...
	p=NextDirEntry(1,&fileSearchMatchX);
	while(p=NextDirEntry(0,&fileSearchMatchX))
	{
#ifndef DISABLE_LONG_FILENAMES
		...
		if(compare((const char*)p->Name, name,11)==0)
			break;
	}
...
int BestBookmark(fileTYPE *file, __uint32_t pm)
{
	int idx,best;
	int32_t bestd,d;
	best=-1;
...
void FileSeek(fileTYPE *file, __uint32_t pos)
{
	__uint32_t p=pos>>9;
	__uint32_t pm=p&~cluster_mask;

	__uint32_t currentsector;
	__uint32_t cluster;
...
void ChangeDirectoryByCluster(__uint32_t cluster)
{
	if(cluster)
	{
...
void ChangeDirectoryByName(const char *name)
{
	DIRENTRY      *p = NULL;        // pointer to current entry in sector buffer
	...
	while(p)
	{
		...
		p=NextDirEntry(0,&fileSearchMatchX);
	}

	if (p)	
	{
		__uint32_t cluster = ConvBB_LE(p->StartCluster);
...
int FindByCluster(__uint32_t parent, __uint32_t cluster)
{
    DIRENTRY      *p = NULL;        // pointer to current entry in sector buffer
...
int ValidateDirectory(__uint32_t directory)
{
